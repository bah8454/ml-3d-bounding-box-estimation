...
"""Parameter to skip the image loading and run the final average error calculations instead."""
fast = True # False

def plot_regressed_3d_bbox(img, truth_img, cam_to_img, box_2d, dimensions, alpha, theta_ray):
    """Plots the 3d bounding box that is obtained from regression.
    Arguments:
    img -- The image to draw bounding boxes upon.
    truth_img -- The ground truth image.
    cam_to_img -- The kitti camera coordinate and localization values
    box_2d -- The 2d bounding box for the given object.
    dimensions -- The dimensions of the 3d bounding box for the given object.
    alpha -- 3d bounding box rotational component.
    theta_ray -- 3d bounding box rotational component.
    """
...
def main():
    """Displays an image, then draws the bounding boxes atop the relevant objects.  Press SPACE to continue.
    """
    final_result = 0
    i = 0
    ...
    for key in sorted(all_images.keys()):
        ...
        for detectedObject in objects:
            ...
            final_result += np.linalg.norm(np.array(location) - np.array(label['Location']))
            i += 1
            print(i)
            print(final_result)
            print('-------------')
